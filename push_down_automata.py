...
class AutomataRuby:
    ...
    def estados(self, simbolo, stack, count):
        try:
            ...
            if re.search('^print',simbolo) or re.search('^puts ',simbolo):
                ...
                self.estados(simbolo,stack,self.states)
            elif re.search('^\w+ =', simbolo):
                print('entra')
                stack.apilar("variable")
                self.states = count+1
                self.estados(re.sub('^\w+ ','',simbolo),stack,self.states)
            elif re.search('^= .+', simbolo):
                print('sale')
                stack.desapilar()
            elif re.search('^(?!println\()\w+\(',simbolo):
                print('entra')
                stack.apilar("funcion")
                self.states = count+1
                self.estados(re.sub('^(?!println\()\w+','',simbolo),stack,self.states)
            elif (re.search('^\(.+\)$', simbolo) or re.search('^".+"$', simbolo)or re.search('^ .+$', simbolo)) \
                    and len(stack.items)!=0 and(stack.items[len(stack.items)-1]=='salida'
                                                or stack.items[len(stack.items)-1]=='funcion'):
                print('sale')
                stack.desapilar()
            elif re.search('^if\s+[^;\n]+;?', simbolo):
                print('entra')
                stack.apilar("condicional")
    ...
    def analizar(self, codigo):
        ...
        for i,simbolo in enumerate(codigo):
            ...
            self.estados(simbolo , pila, self.states)
    ...
    def estados(self, simbolo, stack, count):
        ...
        try:
            if re.search('^println|print',simbolo):
                stack.apilar("salida")
                self.states = count+1
                self.estados(simbolo.replace('println',''),stack,self.states)
                ...
                self.states = count+1
                self.estados(re.sub('^(?!println\()\w+','',simbolo),stack,self.states)
            elif re.search('^\(.+\)$', simbolo) and len(stack.items)!=0 and(stack.items[len(stack.items)-1]=='salida' or stack.items[len(stack.items)-1]=='funcion'):
                stack.desapilar()
            elif re.search('^if\s+[^;\n]+;?', simbolo):
                stack.apilar("condicional")
